...
Now the data needs to be summed up by the `PDAT` -- GEO publication date variable and are ready to be plotted.
Sequencing for expression profiling has seriously taken off as the number of datasets in NCBI GEO is accumulating exponentially (Figure \@ref(fig:geocount)).

```{r geocount, fig.cap="The number of RNA-seq series and associated primary publications submitted to NCBI GEO database grows exponentially."}
library(lubridate)
pdat <- ds %>% 
...
```{r pubdiff, fig.cap="Length of timeperiod between publication dates of GEO HT-sequencing dataset and article including the study. Left, distribution of timeperiods. Right, scatterplot of dataset versus article publication dates."}
library(grid)
library(gridExtra)
h <- ggplot(merged_ds, aes(pubdiff)) +
  ...
  ylab("Number of GEO Series") #+
  # ggtitle("Timeperiod between article and GEO dataset publication dates")
